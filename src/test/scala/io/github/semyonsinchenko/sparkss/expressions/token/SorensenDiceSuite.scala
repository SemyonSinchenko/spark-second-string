...
class SorensenDiceSuite extends AnyFunSuite {

  private def score(left: String, right: String): Double = {
    SorensenDice.similarity(UTF8String.fromString(left), UTF8String.fromString(right))
  }
  ...
  test("partial overlap should return expected score") {
    assert(score("a b c", "a b d") === (4.0 / 6.0))
  }
