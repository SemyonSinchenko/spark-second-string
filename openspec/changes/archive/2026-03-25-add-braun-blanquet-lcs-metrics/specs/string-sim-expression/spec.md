...
- **THEN** it SHALL conform to the unified binary expression contract
- **THEN** the contract SHALL support normalized matrix metrics including `levenshtein` and `lcs_similarity`
...
- **THEN** parity validation SHALL include `jaccard`, `sorensen_dice`, `overlap_coefficient`, `cosine`, `braun_blanquet`, `levenshtein`, and `lcs_similarity`
