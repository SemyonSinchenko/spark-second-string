...
- **THEN** it SHALL conform to the unified binary expression contract
- **THEN** the contract SHALL support normalized matrix metrics including `levenshtein`, `lcs_similarity`, `jaro`, `jaro_winkler`, and `needleman_wunsch`
...
- **THEN** parity validation SHALL include `jaccard`, `sorensen_dice`, `overlap_coefficient`, `cosine`, `braun_blanquet`, `levenshtein`, `lcs_similarity`, `jaro`, `jaro_winkler`, and `needleman_wunsch`
